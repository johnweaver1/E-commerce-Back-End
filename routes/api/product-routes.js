  ...
  Product.create(req.body)
    .then((product) => {
      ...
      if (req.body.tagIds.length) {
        const productTIdArr = req.body.tagIds.map((tag_id) => {
          return {
            product_id: product.id,
            tag_id,
          };
        });
        return ProductTag.bulkCreate(productTIdArr);
      }
      // if no product tags, just respond
      res.status(200).json(product);
    })
    .then((productTIds) => res.status(200).json(productTIds))
    .catch((err) => {
      console.log(err);
...
router.put('/:id', (req, res) => {
  ...
  Product.update(req.body, {
    ...
    .then((productTags) => {
      ...
      const productTagRemoval = productTags
        .filter(({ tag_id }) => !req.body.tagIds.includes(tag_id))
        .map(({ id }) => id);

      // run both actions
      return Promise.all([
        ProductTag.destroy({ where: { id: productTagRemoval } }),
